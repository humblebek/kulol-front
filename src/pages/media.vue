                        ...
                        <div v-for="(media, index) in mediaStore.medias" :key="index" class="rounded-2xl overflow-hidden  border-2 border-amber-700">
                          <a class="relative block" :href="safeGet(media, 'link', '#')" target="_blank">
                                <img
                                    ...
                                    :src="storageUrl(safeGet(media, 'image', ''))"
                                    alt=""
                                />

                                <!-- Overlay: butun konteynerni egallaydi va markazlaydi -->
                                <span class="absolute inset-0 grid place-items-center z-10 pointer-events-none">
                                    <!-- Ikona: doira + glyph; grid/leading-none bilan optik markaz -->
                                    <span class="mdi mdi-play inline-grid place-items-center
                                                w-12 h-12 rounded-full border-2 border-amber-200
                                                text-4xl leading-none">
                                    </span>
                                </span>
                            </a>
                            <h3 class="text-xl mt-2 p-2">
                                {{ safeGet(media, `translations.${$i18n.locale}.title`) }}
                            </h3>
                            <!-- <iframe class="w-full aspect-video" :src="media.link" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe> -->
                        </div>
                    </div>

                     <div class="text-center mt-7 md:mt-12" v-if="safeGet(mediaStore, 'pagination.current_page', 0)  < safeGet(mediaStore, 'pagination.last_page', 1)">
                        <button class="btn btn-outline" @click="getData({...mediaStore.pagination, page: mediaStore.pagination.current_page + 1})" data-text="Load More">
